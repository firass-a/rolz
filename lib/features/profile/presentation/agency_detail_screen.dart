  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (agency == null) {
      return Scaffold(
        ...
        appBar: CustomAppBar(title: AppStrings.agency),
        body: Center(
          child: ErrorState(
            icon: Iconsax.buildings,
            title: AppStrings.agencyNotFound,
            subtitle: AppStrings.agencyNotFoundSubtitle,
          ),
        ),
      );
    }
      ...
      body: CustomScrollView(
        slivers: [
          _CoverHeader(
            ...
            onShare: () => context.showSnack(AppStrings.shareLinkCopied(agency.name)),
          ),
          SliverToBoxAdapter(
            child: Padding(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      if (agency.isVerified) VerifiedBadge(label: AppStrings.talentVerified),
                    ],
                  ...
                  Row(
                    children: [
                      Expanded(child: KrStatCard(icon: Iconsax.profile_2user, value: '${agency.talentCount}', label: AppStrings.talents)),
                      const SizedBox(width: AppSpacing.sm),
                      Expanded(child: KrStatCard(icon: Iconsax.star_1, value: Formatters.formatRating(agency.rating), label: AppStrings.rating, animateCounter: false)),
                      const SizedBox(width: AppSpacing.sm),
                      Expanded(child: KrStatCard(icon: Iconsax.calendar, value: '${agency.createdAt.year}', label: AppStrings.since, animateCounter: false)),
                    ],
                  ).animate().fadeIn(delay: 80.ms, duration: 350.ms),
                  const SizedBox(height: AppSpacing.xl),
                  if (agency.description.isNotEmpty) ...[
                    Text(AppStrings.about, style: AppTextStyles.subsectionTitle),
                    const SizedBox(height: AppSpacing.md),
                    Text(agency.description, style: AppTextStyles.bodyMuted),
                    const SizedBox(height: AppSpacing.xl),
                  ],
                  if (agency.specialties.isNotEmpty) ...[
                    Text(AppStrings.specialties, style: AppTextStyles.subsectionTitle),
                    const SizedBox(height: AppSpacing.md),
                    Wrap(
                      ...
                      children: agency.specialties
                          .map((s) => Container(
                                ...
                                child: Text(DisplayLocalizer.t(s), style: AppTextStyles.bodySmall.copyWith(color: AppColors.textPrimary)),
                              ))
                          .toList(),
                    ),
                    const SizedBox(height: AppSpacing.xl),
                  ...
                  if (agency.website.isNotEmpty) ...[
                    GestureDetector(
                      onTap: () => context.showSnack(AppStrings.openingPlatform(agency.website)),
                      child: Row(
                  ...
                  KrSectionHeader(title: AppStrings.ourTalents, subtitle: AppStrings.representedCount(talents.length), padding: EdgeInsets.zero),
                  const SizedBox(height: AppSpacing.md),
                  talents.isEmpty
                      ? EmptyState(
                          icon: Iconsax.profile_2user,
                          title: AppStrings.noTalentsYet,
                          subtitle: AppStrings.noTalentsYetSubtitle,
                          compact: true,
                        )
                      : GridView.builder(
                          shrinkWrap: true,
          ...
          children: [
            ...
            PositionedDirectional(
              ...
              child: Container(
                ...
                child: KrNetworkImage(imageUrl: agency.logoUrl, width: 80, height: 80, errorIcon: Iconsax.buildings),
              ),
            ),
          ],
        ),
